...
class BilImage extends JFrame implements ActionListener {
	...
	public static void main(String s[]) {
		...
		frame.setTitle("Maximum Likelihood Classifier (Single Class Extraction): SMIC Tool");
...
/////////// *********************** Class Read Image Data To Read The Header
/////////// File of Image. ***********///////////////
...
class ReadImageData {
	...
	public void read(File Name)// read image file name
	{
		...
		try {
			...
			System.out.println("Image can not be read");
		}
...
class picture extends JPanel {
	...
	public BufferedImage getpicdata(int Color1, int Color2, int Color3)/// return
																		/// image
	{
		...
		System.out.println("color1 is " + p + "color2 is " + q + "color3 is " + r);
		...
		try {
			...
			FileInputStream f1 = new FileInputStream(ImageFile); // reading
																	// pixel
																	// values
																	// for each
																	// band
			...
			System.out.println("Before Database creation : " + e);
		}
		...
		try {
			...
			// when size of image is smaller than window size set by x_win &
			// y_win
			if (flag1 == 0) {
			...
			else if (flag1 == 1) // when size of image is greater than window
									// size set by x_win & y_win
			{
				...
				if (flag1 == 0) {
					...
					if (evt.getClickCount() > 0) {
						if (y > (height) || x > (width) || y < 0 || x < 0) {
							...
							JOptionPane.showMessageDialog(null, "You clicked outside the image", "Warnning Message",
...
/////////////// ******************* Zoom In Class For Enlargement of The
/////////////// Image****************///////////////
...
class calculate {
	...
	static int[] xy = new int[2]; // for tranfering x,y coordinate for accuracy
									// assesment
	...
	public calculate(int x, int y) {
		...
		for (int i = 0; i < Bands; i++)
			banddata[i] = pixvalue[i][newy][newx]; // for tranfring value to
													// acuracy class
		...
		try {
			...
			for (int m = 0; m < l; m++) {
				...
				pixel_Array[m] = pixelArray[m].doubleValue(); // converting
																// double object
																// to primitive
																// double**/
	...
	public double[] retbandsdata()// return the total of pixel values of
									// coordinates clicked
	{
		return bandsdata;
	}
...
class Enhancement {
	...
	public void enhances(int col1, int col2, int col3) {
		 ...
		 * else {
		 *
		 * byte pix_rgb[][]; int []minBand=new int[Bands]; int []maxBand=new
		 * int[Bands]; File ImageFile; ReadImageData ImgData1;
		 *
		 * System.gc(); ImgData1 = new ReadImageData(); ImageFile =
		 * ImgData1.imagefile(); int origin_x=0; int origin_y=600; int y_pass2;
		 * ImgData1.read(ImageFile); Bands = ImgData1.bands(); Width =
		 * ImgData1.columns(); Height = ImgData1.rows(); int p = col1; int q =
		 * col2; int r = col3; System.out.println("Bands : " + Bands);
		 * System.out.println("Width : " + Width); System.out.println(
		 * "Height : " + Height); System.gc(); int i=0, x, j=0, l, k; int index
		 * = 0;
		 *
		 * int pix[]; int pix1[]; int pix2[]; int pix3[];
		 *
		 * int pix_2[]; int pix1_2[]; int pix2_2[]; int pix3_2[];
		 *
		 *
		 * int p1, p2; int z,g; char temp[] = new char[10]; char temp2[] = new
		 * char[100]; int myFlag = 0;
		 *
		 * int maxRGB = 0, tempMax = 0; byte temp_max[];
		 *
		 *
		 * try { InputStream MyHdr = new FileInputStream(ImageFile + ".hdr");
		 * for (k = 0; k < 71; k++) temp2[k] = (char)MyHdr.read(); for (k = 71;
		 * k < 78; k++) { temp[k - 71] = (char)MyHdr.read(); } } catch
		 * (Exception e) { myFlag = 1; } String myTemp = (new
		 * String(temp)).trim(); System.out.println("Read Data : " + myTemp);
		 * try { String s = myTemp.substring(1); picture.shiftFlag =
		 * Integer.parseInt (s); } catch(Exception e){ myFlag = 1;}
		 *
		 * System.out.println("My Shift : " + picture.shiftFlag); if(myFlag ==
		 * 1) picture.shiftFlag = 8;
		 *
		 * long fsize, fsize2;
		 *
		 * MappedByteBuffer mBuf; int tempWidth;
		 *
		 * try {
		 *
		 *
		 *
		 *
		 * System.gc(); } catch (Exception e) { System.out.println(
		 * "Before Database creation : " + e); }
		 *
		 *
		 *
		 *
		 ...
		 *
		 *
		 * FileInputStream fin_band[]=new FileInputStream[Bands]; FileChannel
		 * fchan_in[]=new FileChannel[Bands];
		 *
		 * index = 0; double myTempCalc;
		 *
		 * int slices, y_pass, y_origin; int
		 * x_win=Width,y_win=600,y_win2,flag1=0; int uneven_flag = 1;
		 *
		 * if((Height % y_win) == 0) { slices = (int)(Height/y_win); uneven_flag
		 * = 0; } else slices = (int)(Height/y_win)+1;
		 *
		 * fsize2=((Height-(slices-1)*y_win)*x_win); y_win2 =
		 * (Height-(slices-1)*y_win); if(Height>y_win || Width>x_win) {
		 * fsize=(y_win*x_win); flag1=1; y_pass=y_win;
		 *
		 * } else { fsize=(Height*Width); System.out.println("Fsize : " +
		 * fsize); flag1=0; y_pass=Width; }
		 *
		 *
		 * int band_num[] = new int[Bands]; String input;
		 *
		 *
		 *
		 * byte pix_byte[],pix_rgb2[][];
		 ...
		 *
		 *
		 * try { for(k=0; k<Bands; k++) { fin_band[k] = new
		 * FileInputStream("Data" + BilImage.name + k); fchan_in[k] =
		 * fin_band[k].getChannel();
		 *
		 * }
		 *
		 *
		 * if(picture.shiftFlag == 8) { pix = new int [(int)fsize]; pix1 = new
		 * int [(int)fsize]; pix2 = new int [(int)fsize]; pix3 = new int
		 * [(int)fsize];
		 *
		 * pix_2 = new int [(int)fsize2]; pix1_2 = new int [(int)fsize2]; pix2_2
		 * = new int [(int)fsize2]; pix3_2 = new int [(int)fsize2];
		 *
		 * pix_rgb=new byte[Bands][(int)fsize]; pix_rgb2=new
		 * byte[Bands][(int)fsize2]; } else { pix = new int [(int)fsize]; pix1 =
		 * new int [(int)fsize]; pix2 = new int [(int)fsize]; pix3 = new int
		 * [(int)fsize];
		 *
		 * pix_2 = new int [(int)fsize2]; pix1_2 = new int [(int)fsize2]; pix2_2
		 * = new int [(int)fsize2]; pix3_2 = new int [(int)fsize2];
		 *
		 * pix_rgb=new byte[Bands][(int)fsize*2]; pix_rgb2=new
		 * byte[Bands][(int)fsize2*2];
		 *
		 * }
		 *
		 *
		 * byte pix_line[]; byte pix_line2[]; int abc=0; if(picture.shiftFlag ==
		 * 8) { pix_line = new byte[x_win]; pix_line2 = new byte[x_win]; abc =
		 * x_win; } else { pix_line = new byte[x_win * 2]; pix_line2 = new
		 * byte[x_win * 2]; abc = x_win * 2; } int line_start,line_start2=0; int
		 * myMax = 0; int myMin=0; long count;
		 *
		 * int tempCount; if(picture.shiftFlag == 8) count = fsize; else count =
		 * fsize * 2;
		 *
		 *
		 * long counter=0; long start_pos = 0; maxBand=new int[Bands];
		 * minBand=new int[Bands]; int maxBand2[]=new int[Bands]; int
		 * minBand2[]=new int[Bands]; int myMin2=0; int myMax2=0; line_start =
		 * 0; int first_time=1; System.out.println(
		 * "Calculating Maximum and Minimum1"); for(i=0; i<Bands; i++) {
		 * maxBand[i] = -1; minBand[i] = 65535; } System.out.println(
		 * "Calculating Maximum and Minimum2");
		 *
		 * //Calculation of Maximum and Minimum FileInputStream f1 = new
		 * FileInputStream(ImageFile); FileChannel fchan = f1.getChannel();
		 *
		 * if(picture.shiftFlag == 8) tempWidth = Width; else tempWidth = Width
		 * * 2; System.out.println("Calculating Maximum and Minimum"); for(j=0;
		 * j<Height; j++) {
		 *
		 * for(i=0; i<Bands; i++) { mBuf =
		 * fchan.map(FileChannel.MapMode.READ_ONLY,line_start,tempWidth);
		 * mBuf.get(pix_line); for(k=0; k<tempWidth; k++) pix_rgb[i][k] =
		 * pix_line[k]; line_start += tempWidth;
		 *
		 * for(k=0; k<Width; k++) { if(picture.shiftFlag != 8) { p1 =
		 * pix_rgb[i][2*k] << 24; p2 = (pix_rgb[i][2*k+1] <<24 ) >>> 8;
		 * tempCount = p1 | p2; tempCount = tempCount >>> 16; } else tempCount =
		 * (pix_rgb[i][k] << 24) >>> 24;
		 *
		 * if(tempCount>maxBand[i]) maxBand[i]=tempCount;
		 *
		 * if(tempCount<minBand[i]) minBand[i]=tempCount; } } if(j%500 == 0) {
		 * System.out.println("j : " + j); System.gc(); } }
		 *
		 *
		 *
		 * myMax = maxBand[0]; for(i=1; i<Bands; i++) if(myMax < maxBand[i])
		 * myMax = maxBand[i];
		 *
		 * for(i=0;i<Bands;i++) System.out.println("the maximum value for band"+
		 * (i+1 )+ " " + maxBand[i]);
		 *
		 * for(i=0;i<Bands;i++) System.out.println("the minimum value for band"+
		 * (i+1) + " " + minBand[i]);
		 *
		 *
		 * // when size of image is smaller than window size set by x_win &
		 * y_win if(flag1==0) {
		 *
		 * for(i=0; i<Bands; i++) { if(picture.shiftFlag == 8)
		 * mBuf=fchan_in[i].map(FileChannel.MapMode.READ_ONLY,0,fsize); else
		 * mBuf=fchan_in[i].map(FileChannel.MapMode.READ_ONLY,0,fsize*2);
		 * mBuf.get(pix_rgb[i]); } System.gc();
		 *
		 * //convert raw bytes into arrays. if(picture.shiftFlag == 8) {
		 * System.out.println("This is an 8 bit image");
		 * for(i=0;i<Height*Width;i++) { pix[i]=255; if(Bands!=1) {
		 *
		 * if(pix_rgb[p-1][i]>0) pix1[i]= (int
		 * )((pix_rgb[p-1][i]-minBand[p-1])/(maxBand[p-1]-minBand[p-1]))*255;
		 * else pix1[i]=
		 * (int)((256+pix_rgb[p-1][i]-minBand[p-1])/(maxBand[p-1]-minBand[p-1]))
		 * *255;
		 *
		 *
		 *
		 * if(pix_rgb[q-1][i]>0) pix2[i]=
		 * (int)((pix_rgb[q-1][i]-minBand[q-1])/(maxBand[q-1]-minBand[q-1]))*
		 * 255; else pix2[i]=
		 * (int)((256+pix_rgb[q-1][i]-minBand[q-1])/(maxBand[q-1]-minBand[q-1]))
		 * *255;
		 *
		 *
		 * if(pix_rgb[r-1][i]>0) pix3[i]=
		 * (int)((pix_rgb[r-1][i]-minBand[r-1])/(maxBand[r-1]-minBand[r-1]))*
		 * 255; else pix3[i]=
		 * (int)((256+pix_rgb[r-1][i]-minBand[r-1])/(maxBand[r-1]-minBand[r-1]))
		 * *255;
		 *
		 *
		 * }
		 ...
		 * else {
		 *
		 * if(pix_rgb[0][i]>0) { pix1[i]=
		 * ((pix_rgb[0][i]-minBand[0])/(maxBand[0]-minBand[0]))*255;
		 * pix2[i]=pix1[i]; pix3[i]=pix1[i]; } else { pix1[i]=
		 * ((256+pix_rgb[0][i]-minBand[0])/(maxBand[0]-minBand[0]))*255;
		 * pix2[i]=pix1[i]; pix3[i]=pix1[i]; } } }//close for } else {
		 *
		 * for(i=0;i<fsize;i++) { pix[i]=255; if(Bands!=1) { //for pix1
		 * if(pix_rgb[p-1][2*i]<0) z=256+pix_rgb[p-1][2*i]; else
		 * z=pix_rgb[p-1][2*i];
		 *
		 * if(pix_rgb[p-1][2*i+1]<0) g=256+pix_rgb[p-1][2*i+1]; else
		 * g=pix_rgb[p-1][2*i+1];
		 *
		 * p1 = z << 8; p2 = g; pix1[i] = p1 | p2;
		 *
		 * myTempCalc = ((double)pix1[i] / (double)myMax) * 255.0; pix1[i] =
		 * (int)(myTempCalc-minBand[p-1])/(maxBand[p-1]-minBand[p-1])*255;
		 *
		 * // for pix2 if(pix_rgb[q-1][2*i]<0) z=256+pix_rgb[q-1][2*i]; else
		 * z=pix_rgb[q-1][2*i];
		 *
		 * if(pix_rgb[q-1][2*i+1]<0) g=256+pix_rgb[q-1][2*i+1]; else
		 * g=pix_rgb[q-1][2*i+1];
		 *
		 * p1 = z << 8; p2 = g; pix2[i] = p1 | p2;
		 *
		 * myTempCalc = ((double)pix2[i] / (double)myMax) * 255.0; pix2[i] =
		 * (int)(myTempCalc-minBand[q-1])/(maxBand[q-1]-minBand[q-1])*255;;
		 *
		 * // for pix3
		 *
		 * if(pix_rgb[r-1][2*i]<0) z=256+pix_rgb[r-1][2*i]; else
		 * z=pix_rgb[r-1][2*i];
		 *
		 * if(pix_rgb[r-1][2*i+1]<0) g=256+pix_rgb[r-1][2*i+1]; else
		 * g=pix_rgb[r-1][2*i+1];
		 *
		 * p1 = z << 8; p2 = g; pix3[i] = p1 | p2;
		 *
		 * myTempCalc = ((double)pix3[i] / (double)myMax) * 255.0; pix3[i] =
		 * (int)(myTempCalc-minBand[r-1])/(maxBand[r-1]-minBand[r-1])*255;
		 *
		 * }
		 *
		 * else {
		 ...
		 * }
		 *
		 * }//close for System.out.println("Image Complete"); }
		 *
		 ...
		 *
		 *
		 * }//close if else if(flag1==1) // when size of image is greater than
		 * window size set by x_win & y_win {
		 *
		 * System.out.println("the maximum value in bigger image is:"+ myMax);
		 *
		 * System.out.println("Maximum Value : " + myMax);
		 *
		 * if(picture.shiftFlag==8) { line_start=0; for(int v=0;v<slices;v++) {
		 * System.out.println("enter v :"+v);
		 *
		 * if(v!=(slices-1)) {
		 *
		 * for( i=0;i<Bands;i++) { if(Bands==1 && i!=0) { continue; }
		 *
		 * //line_start=line_start2; line_start=v*y_win*Width;
		 * System.out.println("line start i :"+line_start+" "+i); int k_index=0;
		 * // read the files line by line i.e x_win bytes in one go // and
		 * repeat unpo y_win i.e. size of window for(j=0;j<y_win;j++) {
		 * mBuf=fchan_in[i].map(FileChannel.MapMode.READ_ONLY,line_start,x_win);
		 * mBuf.get(pix_line);// store each line in separate buffer // i.e
		 * pix_line[] line_start=line_start+Width;
		 *
		 * // increment line_start by Width to point to next line // and leave
		 * rest of the pixels for(k=0;k<x_win;k++,k_index++) { // transfer one
		 * line data into pix_rgb[][] pix_rgb[i][k_index]=pix_line[k];
		 *
		 * } }//close for
		 *
		 * System.out.println("i :"+i);
		 ...
		 * }//close for
		 *
		 * //line_start2=line_start; System.out.println("enter ");
		 * System.out.println("fsize :"+fsize);
		 *
		 * if(Bands!=1) { for( i=0;i<fsize;i++) { pix[i]=255;
		 * if(pix_rgb[p-1][i]>0) pix1[i]=
		 * (pix_rgb[p-1][i]-minBand[p-1])/(maxBand[p-1]-minBand[p-1])*255; else
		 * pix1[i]=
		 * (256+pix_rgb[p-1][i]-minBand[p-1])/(maxBand[p-1]-minBand[p-1])*255;
		 *
		 * if(pix_rgb[q-1][i]>0) pix2[i]=
		 * (pix_rgb[q-1][i]-minBand[q-1])/(maxBand[q-1]-minBand[q-1])*255; else
		 * pix2[i]=
		 * (256+pix_rgb[q-1][i]-minBand[q-1])/(maxBand[q-1]-minBand[q-1])*255;
		 *
		 * if(pix_rgb[r-1][i]>0) pix3[i]=
		 * (pix_rgb[r-1][i]-minBand[r-1])/(maxBand[r-1]-minBand[r-1])*255; else
		 * pix3[i]=
		 * (256+pix_rgb[r-1][i]-minBand[r-1])/(maxBand[r-1]-minBand[r-1])*255;
		 *
		 *
		 * }
		 *
		 ...
		 * raster.setSamples(0,origin_y,x_win,y_win,1,pix2);
		 * raster.setSamples(0,origin_y,x_win,y_win,2,pix3);
		 * raster.setSamples(0,origin_y,x_win,y_win,3,pix); System.out.println(
		 * "slice displayed"); origin_x=0; origin_y=origin_y+y_win; System.gc();
		 *
		 * } else { for( i=0;i<fsize;i++) { pix[i]=255; if(pix_rgb[0][i]>0)
		 * pix1[i]= (pix_rgb[0][i]-minBand[0])/(maxBand[0]-minBand[0])*255; else
		 ...
		 * raster.setSamples(0,origin_y,x_win,y_win,3,pix);
		 *
		 * System.out.println("slice displayed"); origin_x=0;
		 * origin_y=origin_y+y_win;
		 *
		 * } System.gc();
		 *
		 * }//close if(v!=num_of_slice-1)
		 *
		 * else if(v==(slices-1)) { System.out.println(
		 * "v origin_y line_start  :"+v+" "+origin_y+" "+line_start); for(
		 * i=0;i<Bands;i++) { if(Bands==1 && i!=0) { continue; }
		 * line_start=v*y_win*Width; System.out.println("line start2 i :"
		 * +line_start+" "+i); int k_index=0; // read the files line by line i.e
		 * x_win bytes in one go // and repeat unpo y_win i.e. size of window
		 *
		 * for(j=0;j<(Height-(slices-1)*y_win);j++) {
		 * mBuf=fchan_in[i].map(FileChannel.MapMode.READ_ONLY,line_start,x_win);
		 * mBuf.get(pix_line);// store each line in separate buffer // i.e
		 * pix_line[] line_start=line_start+Width; // increment line_start by
		 * Width to point to next line // and leave rest of the pixels
		 * for(k=0;k<x_win;k++,k_index++) { // transfer one line data into
		 * pix_rgb[][] pix_rgb2[i][k_index]=pix_line[k];
		 *
		 * }
		 *
		 * }//close for System.out.println("i :"+i); }//close for
		 *
		 * System.out.println("enter "); System.out.println("fsize2 :"+fsize2);
		 *
		 * if(Bands!=1) { for( i=0;i<fsize2;i++) { pix_2[i]=255;
		 *
		 * if(pix_rgb2[p-1][i]>0) pix1_2[i]=
		 * (pix_rgb2[p-1][i]-minBand[p-1])/(maxBand[p-1]-minBand[p-1])*255; else
		 * pix1_2[i]=
		 * (256+pix_rgb2[p-1][i]-minBand[p-1])/(maxBand[p-1]-minBand[p-1])*255;
		 *
		 * if(pix_rgb2[q-1][i]>0) pix2_2[i]=
		 * (pix_rgb2[q-1][i]-minBand[q-1])/(maxBand[q-1]-minBand[q-1])*255; else
		 * pix2_2[i]=
		 * (256+pix_rgb2[q-1][i]-minBand[q-1])/(maxBand[q-1]-minBand[q-1])*255;
		 *
		 * if(pix_rgb2[r-1][i]>0) pix3_2[i]=
		 * (pix_rgb2[r-1][i]-minBand[r-1])/(maxBand[r-1]-minBand[r-1])*255; else
		 * pix3_2[i]=
		 * (256+pix_rgb2[r-1][i]-minBand[r-1])/(maxBand[r-1]-minBand[r-1])*255;
		 * } y_pass2=(Height-origin_y);//(num_of_slice-1));
		 * raster.setSamples(0,origin_y,x_win,y_pass2,0,pix1_2);
		 * raster.setSamples(0,origin_y,x_win,y_pass2,1,pix2_2);
		 * raster.setSamples(0,origin_y,x_win,y_pass2,2,pix3_2);
		 * raster.setSamples(0,origin_y,x_win,y_pass2,3,pix_2);
		 *
		 * } else { for( i=0;i<fsize2;i++) { pix_2[i]=255;
		 *
		 * if(pix_rgb2[0][i]>0) pix1_2[i]=
		 * (pix_rgb2[0][i]-minBand[0])/(maxBand[0]-minBand[0])*255; else
		 * pix1_2[i]=
		 * (256+pix_rgb2[0][i]-minBand[0])/(maxBand[0]-minBand[0])*255; }
		 * pix3_2=pix2_2=pix1_2;
		 *
		 * y_pass2=(Height-origin_y);//(num_of_slice-1));
		 * raster.setSamples(0,origin_y,x_win,y_pass2,0,pix1_2);
		 ...
		 */
		/*
		 * line_start=0; System.out.println("Total number of slices : " +
		 * slices); for(int v=0;v<slices;v++) { System.out.println(
		 * "\nSlice Number v :"+v);
		 *
		 * if(v!=(slices-1)) {
		 *
		 * for( i=0;i<Bands;i++) { if(Bands==1 && i!=0) { continue; }
		 *
		 * line_start=v*y_win*Width*2; System.out.println("line start i :"
		 * +line_start+" "+i); System.out.println("\n"); int k_index=0; // read
		 * the files line by line i.e x_win bytes in one go // and repeat unpo
		 * y_win i.e. size of windowx for(j=0;j<y_win;j++) {
		 * mBuf=fchan_in[i].map(FileChannel.MapMode.READ_ONLY,line_start,x_win*2
		 * ); mBuf.get(pix_line);// store each line in separate buffer
		 * for(k=0;k<x_win*2;k++,k_index++) { pix_rgb[i][k_index]=pix_line[k];
		 *
		 * }
		 *
		 * line_start=line_start+(Width*2);
		 ...
		 *
		 *
		 * System.out.println("enter "); System.out.println("fsize :"+fsize);
		 * double TempCalc; int myChkVal;
		 *
		 * System.out.println("Check value : p : " + p + ", q : " + q + ", r : "
		 * + r); if(Bands!=1) { line_start = 0; for( i=0;i<fsize;i++) {
		 * pix[i]=255;
		 *
		 * //for pix1 if(pix_rgb[p-1][2*i]<0) z=256+pix_rgb[p-1][2*i]; else
		 * z=pix_rgb[p-1][2*i];
		 *
		 * if(pix_rgb[p-1][2*i+1]<0) g=256+pix_rgb[p-1][2*i+1]; else
		 * g=pix_rgb[p-1][2*i+1];
		 ...
		 * raster.setSamples(0,origin_y,x_win,y_win,2,pix3);
		 * raster.setSamples(0,origin_y,x_win,y_win,3,pix); System.out.println(
		 * "Main Slice : " + v); origin_x=0; origin_y=origin_y+y_win;
		 * System.gc();
		 *
		 * } else { System.out.println("slice displayed2"); origin_x=0;
		 * origin_y=origin_y+y_win;
		 *
		 * } System.gc();
		 *
		 * }//close if(v!=num_of_slice-1)
		 *
		 * else if(v==(slices-1)) { System.out.println("Last slice"); for(
		 * i=0;i<Bands;i++) { if(Bands==1 && i!=0) { continue; }
		 *
		 * line_start=v*y_win*Width*2; System.out.println("line start i :"
		 * +line_start+" "+i); System.out.println("\n"); int k_index=0;
		 ...
		 * mBuf=fchan_in[i].map(FileChannel.MapMode.READ_ONLY,line_start,x_win*2
		 * ); mBuf.get(pix_line);// store each line in separate buffer
		 * for(k=0;k<x_win*2;k++,k_index++) { pix_rgb2[i][k_index]=pix_line[k];
		 *
		 * }
		 *
		 * line_start=line_start+(Width*2);
		 ...
		 *
		 * System.out.println("Check value : p : " + p + ", q : " + q + ", r : "
		 * + r); if(Bands!=1) { line_start = 0; for( i=0;i<fsize2;i++) {
		 *
		 * pix_2[i]=255;
		 *
		 * //for pix1 if(pix_rgb2[p-1][2*i]<0) z=256+pix_rgb2[p-1][2*i]; else
		 * z=pix_rgb2[p-1][2*i];
		 *
		 * if(pix_rgb2[p-1][2*i+1]<0) g=256+pix_rgb2[p-1][2*i+1]; else
		 * g=pix_rgb2[p-1][2*i+1];
		 *
		 * p1 = z << 8; p2 = g; pix1_2[i] = p1 | p2;
		 *
		 * myTempCalc = ((double)pix1_2[i] / (double)myMax) * 255.0; pix1_2[i] =
		 * (int)myTempCalc;
		 *
		 * // for pix2 if(pix_rgb2[q-1][2*i]<0) z=256+pix_rgb2[q-1][2*i]; else
		 * z=pix_rgb2[q-1][2*i];
		 *
		 * if(pix_rgb2[q-1][2*i+1]<0) g=256+pix_rgb2[q-1][2*i+1]; else
		 * g=pix_rgb2[q-1][2*i+1];
		 *
		 * p1 = z << 8; p2 = g; pix2_2[i] = p1 | p2;
		 *
		 * myTempCalc = ((double)pix2_2[i] / (double)myMax) * 255.0; pix2_2[i] =
		 * (int)myTempCalc;
		 *
		 * // for pix3
		 *
		 * if(pix_rgb2[r-1][2*i]<0) z=256+pix_rgb2[r-1][2*i]; else
		 * z=pix_rgb2[r-1][2*i];
		 *
		 * if(pix_rgb2[r-1][2*i+1]<0) g=256+pix_rgb2[r-1][2*i+1]; else
		 * g=pix_rgb2[r-1][2*i+1];
		 *
		 * p1 = z << 8; p2 = g; pix3_2[i] = p1 | p2;
		 *
		 * myTempCalc = ((double)pix3_2[i] / (double)myMax) * 255.0; pix3_2[i] =
		 * (int)myTempCalc;
		 *
		 * line_start=line_start+Width*2; }
		 *
		 ...
		 * raster.setSamples(0,origin_y,x_win,y_win2,2,pix3_2);
		 * raster.setSamples(0,origin_y,x_win,y_win2,3,pix_2);
		 * System.out.println("slice displayed1"); origin_x=0;
		 * origin_y=origin_y+y_win; System.gc();
		 *
		 * } else {
		 *
		 * System.out.println("slice displayed2"); origin_x=0;
		 * origin_y=origin_y+y_win;
		 *
		 * } System.gc();
		 *
		 ...
		 *
		 *
		 * for( i=0;i<Bands;i++) { fin_band[i].close(); fchan_in[i].close(); } }
		 * }
		 *
...
class imagetable6 extends JFrame implements ActionListener {
	...
	/** array to store final values of co-varience matrix **/
	double p_array[];
	static int state = 2;
	int cls = 0;
	int row1 = 0;
	int flag;
	/**
	 * for storing the returned values from ret_matrixArray of calculate class
	 **/
	...
	public void helpaccuracy(double[] pix) // Function For Calculating Stat
											// Again For Subpixel Acc assesment
	{
		int bands = imgdata8.bands();
		...
		this.pixeldata = pix;
		// **********here is the logic to devlop the varience-covariance
		// matrix**************
		...
		for (int m = 0; m < l1; m++) {
			...
			varience_Array[m] = varienceArray[m].doubleValue(); // converting
																// double object
																// to primitive
																// double**/
		...
		for (int m = 0; m < l2; m++) {
			...
			avarage_Array[m] = avarageArray[m].doubleValue(); // converting
																// double object
																// to primitive
																// double**/
		...
		for (int i = 0; i < pixel[0].length; i++)
			for (int j = 0; j < pixel[0].length; j++) // for removing previous
														// values
				varm[i][j] = 0;
		...
		////////////////// To open a Signature File. ///////////////////////////
			...
			if (r == JFileChooser.APPROVE_OPTION) {
				...
				for (int m = 0; m < len; m++) {
					...
					pixels_array[m] = pixelsarray[m].doubleValue(); // converting
																	// double
																	// object to
																	// primitive
																	// double**/
				...
				for (int j = 0; j < len; j++) {
					Double t = new Double(pixels_array[j]); // for removing
															// pixel values
...
class Fcm extends JFrame implements ActionListener {
	...
	static boolean singleClass = false;
	static int p = 10;
	double sub[][];
	double subT[][];
	public double c[][][];
	...
	String distances[] = { "Euclidean", "V-C", "D.V-C", "Manhattan", "ChessBoard", "Mean-Absolute-Difference",
			"Canberra", "BrayCurtis", "Cosine", "Correlation", "Normalized-Squared-Euclidean",
	...
	JRadioButton mmlc = new JRadioButton("Modified MLC (Single Class)", false);
	...
	public Fcm() {
		...
		mmlc.addActionListener(new ActionListener(){
			@Override
	...
	public void distance_mixture(Object source) {
		...
		if (source == btClasi) {
			...
			try {
				for (int l = 0; l < count; l++) {
					...
					for (int i = 0; i < h * w; i++) {
						...
						// ...............Different Measures of Similarity and
						// Dissimilarity..............
						...
						// ...........NORMALIZED-SQUARED-EUCLIDEAN............................
							...
							for (int j = 0; j < b; j++) {
								mul[0][j] = 0;
								for (int k = 0; k < b; k++) {
									mul[0][j] += ((x[k] - (sum / b) - mean[l][k] + (add / b))
											* (x[k] - (sum / b) - mean[l][k] + (add / b)));
								}
							}
			...
			/// ********* Calculating Meu Array by Distance vector for Fuzzy
			/// C-Means *************////
			 ...
			 *************/
				...
				if (classi == 2) {
					...
					///////// *********Calculating meu Array for Noise
					...
					///////// *********Calculating meu Array for FCM with
					///////// Entropy *************/
						...
						///////// Possibilistic C-Means *********/////////
							...
							if (classi == 5) {
								...
								} else { // Improved possiblistic c means
									if (classi == 7) {
										try {
											for (int j = 0; j < count; j++) {
												for (int i = 0; i < h * w; i++) {
											...
											for (int k = 0; k < count; k++) {
												System.out.println(
														"Eta value for class " + (k + 1) + " : " + arrayEta[k]);
											...
											for (i = 0; i < count; i++) {
												for (k = 0; k < h * w; k++) {
													...
													for (j = 0; j < count; j++) {
														sum += Math.pow(
																((arrayEta[i]
																		* (1 - Math.exp(-dsq[i][k] / arrayEta[i])))
																		/ (arrayEta[j] * (1
																				- Math.exp(-dsq[j][k] / arrayEta[j])))),
																(2 / (m - 1)));
													}
													meu[i][k] = 1 / sum;
													// System.out.println("meu:ipcm:
													// "+meu[i][k]);
											...
											System.out.println(
													"Meu has been Computed for Improved Possibilistic C-Means.");
										} catch (Exception e) {
										...
										try {
											...
											if ((txt.compareTo("Normalized-Squared-Euclidean") == 0)
													|| (txt.compareTo("Canberra") == 0)
													|| (txt.compareTo("Correlation") == 0)) {
												for (int j = 0; j < count; j++) {
													for (int i = 0; i < h * w; i++) {
														dis[j][i] = dsq[j][i];
													}
											...
											// finding determinant of variance
											// covariance matrix
											...
											double minProb[] = new double[count];
											double maxProb[] = new double[count];
											for (int i = 0; i < count; i++){
												minProb[i] = Double.MAX_VALUE;
												maxProb[i] = -Double.MAX_VALUE;
											}
											...
											for (int i = 0; i < h * w; i++) {
												...
												for (int j = 0; j < count; j++) {
													...
													prob[j] *= 1 / Math.pow(det[j], 0.5);
													prob[j] *= Math.pow(Math.E, -0.5 * dis[j][i]);

													minProb[j] = Math.min(minProb[j], Math.min(prob[j], temp));
													maxProb[j] = Math.max(maxProb[j], Math.max(prob[j], temp));
													sum += prob[j];
											...
											for (int j = 0; j < count; j++){
												System.out.println("\nProb["+j+"] : " + prob[j]);
												System.out.println("minProb["+j+"] : " + minProb[j]);
												System.out.println("maxProb["+j+"] : " + maxProb[j]);
											}

											System.out.println(
													"Meu has been Computed for Maximum Likelihood Classifier.");
										} catch (Exception e) {
										...
										System.out.println("\n in 9");
										//Modified Maximum Likelihood Classifier (MLC) Single class
										...
										try {
											...
											//For storing min, max prob per class
											double minProb[] = new double[count];
											double maxProb[] = new double[count];
											for (int i = 0; i < count; i++){
												minProb[i] = Double.MAX_VALUE;
											...
											for (int i = 0; i < h * w; i++) {
												...
												for (int j = 0; j < count; j++) {
													...
													minProb[j] = Math.min(minProb[j], Math.min(prob[j], temp));
													maxProb[j] = Math.max(maxProb[j], Math.max(prob[j], temp));
													//	System.out.println("maxximum value is " + maxProb[j]);
												...
												{
													if (singleClass)
														meu[j][i]= (dsq[j][i]-minProb[j])/(maxProb[j]-minProb[j]);
													else
														meu[j][i] =  p/(dsq[j][i]- minProb[j])/(maxProb[j] - minProb[j]);
											...
											System.out.println(
													"Meu has been Computed for Modified Maximum Likelihood Classifier.");
										} catch (Exception e) {
											System.out.println("MMLC Meu Calculation Error : " + e);
										}
			...
			if (context != 0) {
				...
				try {
					...
					while (t > tend) // while current temprature is greater than
										// minimum temprature limit.
					{
						while (N > iteration) {
							for (int i1 = 0; i1 < count2; i1++)
								System.out.println("Value of Meu(i,j) at pixel " + r1 + "," + c2 + " class " + (i1 + 1)
										+ " : " + meu[i1][r1 * w + c2]);
							...
							for (int i = 0; i < count2; i++) {
								for (int j = 0; j < h * w; j++) {
									...
									if (energyDiff > 0) {
										...
										/*
										 * if(Math.exp(energyDiff/t)>=Math.
										 * random()) { // for(int
										 * j=0;j<count2;j++) {
										 * meu[i][j]=meu1[i][j]; } counter2++; }
										 */
									}
								}
							}
			...
			if (typ == 3) {
				...
				if (r == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						System.out.println("count is" + count); // total number
																// of classes
						...
						for (int k3 = 0; k3 < h * w; k3++) {
							for (int k4 = 0; k4 < count; k4++) {
								...
								m1[k4][k3] = meu[k4][k3]; // making copy of meu
															// array
							}
						}

						for (int k1 = 0; k1 < h * w; k1++) // total no. of
															// pixels
						{
							...
							for (int j = 0; j < count; j++) {
								if (m1[k2][k1] == meu[j][k1]) // Searching for
																// same value
																// which is
																// stored in
																// m1[0][k1],
																// meu array
								{
									k2f = j;// class corresponding to max value
											// is stored
							...
							// f1[k1]=c12; // final array for storing value of
							// class with max value
		...
		/*
		 * System.out.println("value final"+f1[0][0]); System.out.println(
		 * "value final"+f1[1][0]); System.out.println("value final"+f1[2][0]);
		 * System.out.println("value final"+f1[3][0]); System.out.println(
		 * "value final"+f1[4][0]); System.out.println("value final"+f1[5][0]);
		 *
		 * System.out.println("value final"+f1[0][1]); System.out.println(
		 * "value final"+f1[1][1]); System.out.println("value final"+f1[2][1]);
		 * System.out.println("value final"+f1[3][1]); System.out.println(
		 * "value final"+f1[4][1]); System.out.println("value final"+f1[5][1]);
		 *
		 *
		 * System.out.println("value final"+f1[0][6]); System.out.println(
		 * "value final"+f1[1][6]); System.out.println("value final"+f1[2][6]);
		 * System.out.println("value final"+f1[3][6]); System.out.println(
		 * "value final"+f1[4][6]); System.out.println("value final"+f1[5][6]);
		 */
		...
		if (source == openitem) {
			...
			if (r == JFileChooser.APPROVE_OPTION) {
				...
				try {
					InputStream f1 = new FileInputStream(file); // reading pixel
																// values for
																// each band
					try {
						int cd = 1;
	...
	/*
	 * public void itemStateChanged(ItemEvent ie) { JCheckBox
	 * so=(JCheckBox)ie.getItem(); if(so==box4) {
	 *
	 * sa=1; } }
	 */

	/////////// ***********Calculating Contextual for
	/////////// Classifier*************//////////////
	...
	public double contextualEnergy(double[][] meu2) {
		...
		//////////// *************** Classifier With Discontinuity Adaptive
		//////////// Prior ***********/////////
			...
			////////// Noise clustering Without Entropy & With Discontinuity
			////////// Adaptive Prior////////////////
			...
			if (classi == 2) {
				...
				System.out.println(
						"Contextual of Noise clustering Without Entropy & With Discontinuity Adaptive Prior is completed.");
			}

			////////// Noise clustering With Entropy & Discontinuity Adaptive
			////////// Prior////////////////
			...
			if (classi == 3) {
				...
				System.out.println(
						"Classification of Noise Clustering With Entropy & Discontinuity Adaptive Prior is Completed.");
			}

			////////// Fuzzy C-Means With Entropy & Discontinuity Adaptive
			////////// Prior////////////////
	...
	public double[][] smoothing(double[][] meu3, double[][] meu4, int count1, int r1, int c1) {
		...
		for (int i1 = 0; i1 < count1; i1++)
			System.out.println("Value of FL at pixel " + r1 + "," + c1 + " class " + (i1 + 1) + " : gama : " + gama
					+ " : " + fl[i1][r1 * w + c1]);
		...
		for (int i1 = 0; i1 < count1; i1++)
			System.out.println(
					"Value of FP at pixel " + r1 + "," + c1 + " class " + (i1 + 1) + " : " + fp[i1][r1 * w + c1]);
		...
		for (int i1 = 0; i1 < count1; i1++)
			System.out.println(
					"Value of FT at pixel " + r1 + "," + c1 + " class " + (i1 + 1) + " : " + ft1[i1][r1 * w + c1]);
